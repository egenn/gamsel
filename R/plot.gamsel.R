plot.gamsel <- function(x,
  ...
  if (is.null(index)) index <- length(x$lambdas)
  ...
  if (is.null(which)) {
    warning("Nothing to plot with this choice of argument which")
    return()
  }
  colvals <- rep("blue",p)
  colvals[lin] <- "green"
  ...
  for (ell in which) {
    o  <- order(x[, ell])
    plot(x[o,ell], termmat[o,ell],
         type = 'l',
         ylab = paste("f(v", ell,")", sep = ""),
         xlab = paste("v", ell, sep = ""),
         ylim = ylims,
         col = colvals[ell],
         lwd = 2, ...)
