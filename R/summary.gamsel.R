summary.gamsel <- function(object, label = FALSE, ...) {
  ...
  plot(0, type = "n", xlab = expression(lambda), ylab = expression(alpha), 
       xlim = c(lambda.max * 1.05, lambda.min * (0.9 - .1*label)), 
       ylim = range(alphas), main = "Linear Components",  log = "x")
  abline(h = 0, lty = 3)
  for (j in seq_len(maxvars)) {
    nzlines(lambda, alphas[j, ], col = colours[j], lwd = 2, type = "l", pch = "", cex = 0)
  }
  if (label) text(rep(lambda.min*.85, maxvars), alphas[, length(lambda)], 
                  labels = seq(maxvars), col = colours, cex = 0.6)
  nbetas <- dim(betas)[1]   
  counter <- rep(seq_len(maxvars), degrees)
  counter <- diag(maxvars)[counter, ]
  nbeta <- sqrt(t(counter) %*% (betas^2))
  betamax <- max(nbeta)
  plot(0, type = "n", xlab = expression(lambda), ylab = expression(paste("||", beta, "||")), 
       xlim = c(lambda.max * 1.05, lambda.min * (0.9 - .1*label)), 
       ylim = c(0,1.05 * betamax), main = "Non-linear Components",  log = "x")
  abline(h = 0,lty = 3)
  for (j in seq_len(maxvars)) {
